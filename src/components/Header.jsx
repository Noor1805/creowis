...
export default function Header() {
  ...
  return (
    <>
      {isMdUp && (
        <div className={`fixed top-0 left-0 w-full z-50 bg-emerald-500 overflow-hidden py-1 transition-transform duration-300 ${
          scrolled ? "-translate-y-full" : "translate-y-0"
        }`}>
          <div className="flex animate-scroll whitespace-nowrap min-w-max justify-center w-full">
            {[...Array(3)].map((_, i) => (
              <div key={i} className="flex items-center text-xs sm:text-sm md:text-base font-medium">
      ...
      {/* Navbar */}
      <header className={`fixed left-0 w-full z-30 flex items-center justify-between px-8 py-4 backdrop-blur-md bg-black/20 transition-all duration-300 ${
        isMdUp 
          ? scrolled 
            ? "top-0" 
            : "top-4" 
        ...
        <nav className="hidden md:flex items-center gap-8 text-white font-medium">
          ...
          <a href="#" className="hover:text-emerald-500 transition-colors">FAQs</a>
